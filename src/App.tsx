...
const ADMOB = [
  { app: 'Nexusia', revenue: 3.10, ecpm: 0.75, impressions: 4133, color: '#7c3aed' },
];
...
function AIWorkerCard({ worker, onToggle, onUpgrade }: { worker: any; onToggle: () => void; onUpgrade: () => void }) {
  const statusColors: Record<string, string> = { ACTIVE: '#10b981', IDLE: '#6b7280', UPGRADING: '#f59e0b' };
  return (
    <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full`} style={{ background: statusColors[worker.status] || '#6b7280' }}/>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        <div className="text-center">
          ...
          <div className="font-mono text-sm text-purple-400">{fmt(worker.baseIncomeRate)}€</div>
        </div>
        <div className="text-center">
      ...
      {worker.unlocked ? (
        <div className="flex gap-2">
          ...
          <button onClick={onUpgrade} className="flex-1 text-xs py-2 rounded-lg bg-purple-600/10 text-purple-400 border border-purple-500/20 font-medium">
            Mejorar ({fmt(worker.costToUpgrade)}€)
          </button>
        </div>
      ) : (
        <button onClick={onToggle} className="w-full text-xs py-2 rounded-lg bg-purple-600/20 text-purple-400 border border-purple-500/30 font-medium">
...
export default function App() {
  ...
  if (!auth) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: '#040608' }}>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-sm">
          <div className="text-center mb-8">
            <div className="w-16 h-16 bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <Brain size={32} className="text-white" />
            </div>
            <h1 className="text-2xl font-bold">Nexusia</h1>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6">
            ...
            <button onClick={() => setAuth(true)} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 rounded-xl font-semibold text-sm">
              Acceder
            </button>
          </div>
        </motion.div>
      ...
      <header className="sticky top-0 z-50 border-b border-white/[0.06] backdrop-blur-xl" style={{ background: 'rgba(4,6,8,0.8)' }}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center text-[10px] font-bold">A</div>
          </div>
        </div>
      </header>
      ...
      {tab === 'dashboard' && (
        <div className="max-w-6xl mx-auto px-4 pb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            <BalanceCard label="Balance" value={state?.balance || 0} icon={Wallet} color="#7c3aed" trend={12.5} />
            <BalanceCard label="Invertido" value={state?.investedCapital || 0} icon={TrendingUp} color="#ec4899" trend={8.2} />
            <BalanceCard label="Retirado" value={state?.totalWithdrawals || 0} icon={ArrowUpRight} color="#10b981" />
            <BalanceCard label="Ganancias Netas" value={state?.netGains || 0} icon={Activity} color="#06b6d4" trend={15.3} />
          </div>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-purple-400"><Counter value={38}/>€</div>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5 mb-6">
            <h3 className="text-sm font-semibold mb-4 flex items-center gap-2">
              <DollarSign size={16} className="text-purple-400" /> Ingresos por Anuncios
            </h3>
        ...
        <div className="max-w-6xl mx-auto px-4 pb-20">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2 bg-white/[0.03] rounded-xl px-3 py-2">
              ...
              <span className="text-xs font-mono text-purple-400">{(state?.aiWorkers || []).filter((w: any) => w.status === 'ACTIVE').length} activos</span>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 pb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5">
              <h3 className="text-sm font-semibold mb-4 flex items-center gap-2">
                <Settings size={16} className="text-purple-400" /> Configuración API
              </h3>
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-purple-400">70/30 (Retiro/Reinversión)</span>
                </div>
                <div className="flex justify-between text-sm">
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5">
              ...
              <div className="text-center py-4">
                ...
                <button onClick={handleReinvest} className="mt-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-2.5 rounded-xl text-sm font-semibold">
        ...
        <div className="max-w-6xl mx-auto px-4 pb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 text-center">
              <div className="text-2xl font-bold text-purple-400"><Counter value={state?.balance || 0}/>€</div>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6">
              ...
              {(['paypal', 'bank', 'reinvest'] as const).map(m => (
                <button key={m} onClick={() => setWithdrawMethod(m)}
                  className={`flex-1 py-2 rounded-xl text-xs font-medium transition-all ${
                    withdrawMethod === m ? 'bg-purple-600/20 text-purple-400 border border-purple-500/30' : 'bg-white/5 text-gray-400 border border-white/10'
                  }`}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleWithdraw} className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-3 rounded-xl text-sm font-semibold">
